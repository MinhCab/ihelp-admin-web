...
const Notifications = (props) => {
    ...
    return (
        <React.Fragment>
            <Popover open={props.opening} anchorEl={props.showNoti} onClose={props.closing}
                anchorOrigin={{
                    vertical: 'bottom',
                    horizontal: 'center',
                }}
            ...
            >
                {
                    NotificationsList.map((noti, index) => {
                        return (
                            <Paper className={classes.paper} elevation={3} key={noti.id} onClick={() => getSelectedValue(noti.id)}>
                                <Grid className={classes.popover} wrap="nowrap" container direction='column' justify="center" alignItems="flex-start" spacing={3} >
